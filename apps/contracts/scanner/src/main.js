...
class ScannerApp {
  ...
  renderProposalCard(proposal) {
    ...
    const activeUsers = this.state.quorumByRegion.get(proposal.region) ?? 0;
    const quorumThreshold = calculateQuorumThreshold(activeUsers, this.state.settings);
      ...
      <article class="proposal-card" data-open-proposal="${proposal.idText}">
        <div class="proposal-card__header">
          <div class="status-row">
            ...
            ${
              proposal.escrow?.state
                ? this.renderStatusPill(proposal.escrow.state)
                : ""
            }
          </div>
        ...
        <p class="card-caption">${escapeHtml(
          proposal.escrow?.state
            ? `${humanizeLabel(proposal.kind)} · ${humanizeLabel(proposal.escrow.state)} escrow`
            : `${humanizeLabel(proposal.kind)} · ${humanizeLabel(proposal.status)}`,
        )}</p>
        ...
        <div class="tag-row">
          ...
          ${
            proposal.backedBy
              ? `<span class="tag-chip">backed</span>`
        ...
        <div class="ledger-grid">
          ...
          <div>
            <span>Quorum</span>
            <strong>${proposal.voterCount}/${quorumThreshold || 0}</strong>
          </div>
          <div>
            <span>Funding</span>
            <strong>${escapeHtml(formatIcp(proposal.requestedFundingE8s))}</strong>
          </div>
        </div>
        ...
        <div class="proposal-card__footer">
          ...
          <span class="meta-inline">
            ${escapeHtml(formatRelative(proposal.createdAt))}
          </span>
        </div>
  ...
  renderDetailSheet() {
    ...
    const quorumThreshold = calculateQuorumThreshold(activeRegionUsers, this.state.settings);
      ...
      <aside class="detail-sheet is-open">
        ...
        <section class="detail-block">
          ...
          <div class="stat-pair-grid">
            ...
            <article>
              <span>Quorum snapshot</span>
              <strong>${proposal.voterCount}/${quorumThreshold}</strong>
