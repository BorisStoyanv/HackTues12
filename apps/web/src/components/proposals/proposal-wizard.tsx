	...
	const handleNext = async () => {
		...
		switch (currentStep) {
			case 0:
				...
				break;
			case 1:
				fieldsToValidate = ["description", "expected_impact"];
				break;
			case 2:
	...
	const onSubmit = async (data: ProposalFormValues) => {
		...
		try {
			...
			try {
				...
				await saveProposalAIDebateClient(
					identity,
					createdProposalId,
					savedDebate,
				);
				...
				console.error(
					"Proposal created but AI debate could not be saved",
					aiError,
				);
			}
			...
			console.error(error);
			const message =
				error instanceof Error ? error.message : "Blockchain failed.";
			...
			>
				...
				<div className="space-y-4">
					...
					<p className="text-xl text-muted-foreground font-medium max-w-lg mx-auto">
						Your proposal has been committed to the ledger and is
						now entering the consensus phase.
					</p>
				</div>
		...
		<div className="w-full">
			<Form {...form}>
				...
				>
					...
					<div className="relative min-h-112.5 flex flex-col">
						<AnimatePresence
							mode="wait"
							custom={direction}
							initial={false}
								...
								>
									<div className="space-y-10">
										<div className="space-y-4">
											...
											/>
											{errors.title && (
												<p className="text-[10px] font-bold text-destructive px-2 uppercase">
									...
									<div className="grid grid-cols-1 gap-12">
										<div className="space-y-4">
											...
											<div className="relative">
												<Textarea
													...
													className={cn(
														"min-h-55 p-8 rounded-[2.5rem] border-border/40 bg-background/50 text-lg leading-relaxed resize-none focus:ring-0 focus:border-foreground transition-all duration-500",
														errors.description
															? "border-destructive"
															: "",
													)}
										...
										<div className="space-y-4">
											...
											<Textarea
												...
												className={cn(
													"min-h-40 p-8 rounded-[2.5rem] border-border/40 bg-background/50 text-lg leading-relaxed resize-none focus:ring-0 focus:border-foreground transition-all duration-500",
							...
							{currentStep === 2 && (
								...
								>
									<div className="space-y-12">
										<div className="p-1 border border-border/20 rounded-[2.5rem] bg-muted/5 overflow-hidden shadow-2xl min-h-112.5">
											<Controller
												name="location"
												control={control}
												render={({ field }) => (
									...
									<div className="grid grid-cols-1 gap-12">
										...
										<div className="space-y-4">
											...
											<Textarea
												...
												className="min-h-55 p-8 rounded-[2.5rem] border-border/40 bg-background/50 text-lg leading-relaxed resize-none focus:border-foreground transition-all duration-500"
									...
									<div className="grid grid-cols-1 gap-12">
										...
										<div className="space-y-4">
											...
											<Textarea
												...
												className="min-h-40 p-8 rounded-[2.5rem] border-border/40 bg-background/50 text-lg leading-relaxed resize-none focus:border-foreground transition-all duration-500"
												{...register(
													"budget_breakdown",
												)}
											/>
