...
export function ProposalFundView({ id, mode, initialData, onBack }: ProposalFundViewProps) {
  ...
  const projectedPercentage = ((proposal.funding_goal ?? 0) > 0) ? (projectedFunding / (proposal.funding_goal ?? 1)) * 100 : 0;

  const handlePledge = () => {
    if (amountNum <= 0) return;
    setIsSubmitting(true);
    // Simulate smart contract interaction and escrow locking
    setTimeout(() => {
  ...
  if (isSuccess) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-4 text-center animate-in fade-in zoom-in duration-500">
        ...
        <h1 className="text-4xl font-black tracking-tight mb-2">Capital Deployed</h1>
        <p className="text-muted-foreground text-xl max-w-md mb-8">
          Your contribution of <span className="text-foreground font-bold">${amountNum.toLocaleString()}</span> has been securely locked in the regional escrow.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-lg mb-8 text-left">
           <div className="p-4 rounded-2xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800">
              <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Impact Score</p>
              <p className="text-xl font-black text-primary">+12.4% Est.</p>
           </div>
           <div className="p-4 rounded-2xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800">
              <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Trust Reward</p>
              <p className="text-xl font-black text-blue-500">+25 $V_p$</p>
           </div>
        </div>

        <div className="flex flex-col sm:flex-row gap-4 w-full max-w-lg">
           <Button className="flex-1 h-12 text-lg font-bold" onClick={() => window.location.href = mode === "authenticated" ? `/dashboard/proposals/${id}` : `/proposals/${id}`}>
              View Updated Proposal
           </Button>
           <Button variant="outline" className="flex-1 h-12 text-lg font-bold" onClick={() => window.location.href = "/dashboard"}>
              Return to Dashboard
           </Button>
        </div>
      </div>
      ...
      {mode === "public" && (
        <header className="z-40 border-b bg-background/80 backdrop-blur-md sticky top-0 h-14">
          <div className="container mx-auto flex h-full items-center px-4 sm:px-6">
            ...
            <h1 className="text-lg font-bold tracking-tight">Deploy Capital</h1>
          </div>
        </header>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 w-full max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-10">
            <div className="space-y-4">
              <Badge variant="outline" className="text-[10px] uppercase font-bold tracking-widest px-2 py-0.5">
              ...
              <p className="text-xl text-muted-foreground leading-relaxed max-w-3xl">
                Your pledge is protected by the OpenFairTrip Escrow. Funds are only released to the community once milestones are verified by on-chain consensus.
              </p>
            </div>
            ...
            <Card className="border-neutral-200 dark:border-neutral-800 shadow-xl overflow-hidden rounded-[2.5rem]">
               <CardHeader className="p-8 md:p-12 pb-4">
                  ...
                  <CardDescription className="text-lg">
                    Define your contribution to this community project.
                  </CardDescription>
               </CardHeader>
               <CardContent className="p-8 md:p-12 pt-0 space-y-8">
                  ...
                  <div className="space-y-6 pt-4">
                    <div className="flex items-center justify-between">
                       <h3 className="font-bold text-lg">Projected Progress</h3>
                       <span className="text-sm font-bold text-muted-foreground tracking-widest uppercase">After Pledge</span>
                    </div>
                    <div className="space-y-3">
                       <div className="flex justify-between text-base">
                          <span className="font-medium text-muted-foreground">Regional Funding</span>
                          <span className="font-black text-xl">
                             {Math.min(100, projectedPercentage).toFixed(1)}%
                          </span>
                       </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="p-6 rounded-3xl bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/50 flex gap-4">
                  ...
                  <div className="space-y-1">
                    <p className="font-bold text-blue-700 dark:text-blue-300">Escrow Security</p>
                    <p className="text-xs text-blue-600/80 dark:text-blue-400/80 leading-relaxed">
                      Funds are cryptographically held. No entity can withdraw capital without meeting community-verified milestones.
                    </p>
                  </div>
               </div>
               <div className="p-6 rounded-3xl bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/50 flex gap-4">
                  ...
                  <div className="space-y-1">
                    <p className="font-bold text-green-700 dark:text-green-300">Verifiable Ledger</p>
                    <p className="text-xs text-green-600/80 dark:text-green-400/80 leading-relaxed">
                      Every deployment is logged on the transparency ledger. Real-time auditing for every dollar spent.
                    </p>
                  </div>
               </div>
            </div>
          ...
          <div className="lg:col-span-5 xl:col-span-4 space-y-8">
            <Card className="border-neutral-200 dark:border-neutral-800 bg-neutral-50/30 dark:bg-neutral-900/30 rounded-[2rem] overflow-hidden">
               <CardHeader className="p-8">
                  <CardTitle className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Milestone Roadmap</CardTitle>
               </CardHeader>
               <CardContent className="px-8 pb-8 space-y-6">
                  {[
                    { phase: 1, title: "Infrastructure", pct: 40 },
                    { phase: 2, title: "Implementation", pct: 40 },
                    { phase: 3, title: "Audit & Close", pct: 20 },
                  ].map((m, i) => (
                    <div key={i} className="flex items-center gap-4 group">
                       ...
                       <div className="flex-1 space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                             ...
                             <span className="text-[10px] font-black text-muted-foreground">{m.pct}% Release</span>
                          </div>
                       </div>
                    </div>
                  ))}
            ...
            <Card className="border-neutral-200 dark:border-neutral-800 rounded-[2rem] p-8 space-y-6 bg-primary text-primary-foreground shadow-2xl">
               <div className="space-y-2">
                  ...
                  <h3 className="text-xl font-bold">Social ROI Projection</h3>
                  <p className="text-sm text-primary-foreground/80 leading-relaxed">
                    Based on your proposed contribution, the project will achieve critical mass <strong>3 weeks faster</strong> than estimated.
                  </p>
               </div>
               <Separator className="bg-white/10" />
               <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                     <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Reach</p>
                     <p className="text-xl font-black">+1,200 Persons</p>
                  </div>
                  <div className="space-y-1">
                     <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Durability</p>
