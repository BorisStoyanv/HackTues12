        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm max-w-2xl leading-relaxed">
              Track the lifecycle of your impact proposals on the Internet Computer.
            </p>
          </div>
          <Link href="/dashboard/proposals/new" className={buttonVariants({ size: "sm", className: "rounded-xl font-bold px-6 shadow-lg shadow-primary/20" })}>
            Create New Draft
          </Link>
        </div>
      </div>
          ...
          {proposals.length > 0 ? (
                  ...
                  <div className="flex flex-col md:flex-row">
                    <div className="p-6 md:p-8 flex-1 space-y-4">
                      <div className="flex items-center gap-3">
                         ...
                         <span className="text-[10px] font-mono text-muted-foreground">ID: {proposal.id}</span>
                      </div>
                    ...
                    <div className="md:w-64 bg-neutral-50 dark:bg-neutral-900/50 border-t md:border-t-0 md:border-l border-neutral-100 dark:border-neutral-800 p-6 md:p-8 flex flex-col justify-center space-y-4">
                       <div className="space-y-1">
                          <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Budget Requirement</p>
                          <p className="text-2xl font-black">${proposal.budget_amount.toLocaleString()}</p>
                       </div>
                       <div className="pt-2">
                          <div className="flex justify-between text-[10px] font-bold uppercase mb-1">
                             <span>Backing</span>
                             <span>{((proposal.yes_weight / (proposal.budget_amount || 1)) * 100).toFixed(0)}%</span>
                          </div>
                          <Progress value={(proposal.yes_weight / (proposal.budget_amount || 1)) * 100} className="h-1.5" />
                       </div>
                    </div>
                  </div>
                </Card>
            ...
            <div className="py-32 text-center space-y-6 bg-neutral-50/50 dark:bg-neutral-900/30 rounded-[3rem] border-2 border-dashed border-neutral-100 dark:border-neutral-900">
               ...
               <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground text-sm max-w-xs mx-auto">Your impact proposals will appear here once you initiate a draft on the blockchain.</p>
               </div>
               <Link href="/dashboard/proposals/new" className={buttonVariants({ variant: "default", className: "rounded-full px-8 shadow-xl shadow-primary/20" })}>
