...
import { Loader2, CheckCircle2, ChevronRight, Shield, Globe, Zap, Landmark, FileText, Building2, MapPin } from "lucide-react";
import { ProposalWizard, WIZARD_STEPS } from "@/components/proposals/proposal-wizard";
...
export default function NewProposalPage() {
  ...
  return (
    <div className="flex h-full w-full bg-background overflow-hidden relative">
      ...
      <div className="hidden lg:flex w-[300px] xl:w-[350px] flex-col bg-muted/20 border-r border-border/40 p-10 justify-between relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 space-y-12">
          <div className="flex items-center gap-3">
             ...
             <span className="text-sm font-black uppercase tracking-[0.2em] italic">Impact Ledger</span>
          </div>
          ...
          <div className="space-y-1">
             ...
             <p className="text-sm text-muted-foreground font-medium pt-4 border-t border-border/40">
                Establish a new regional initiative on the global governance layer.
             </p>
          </div>

          <nav className="space-y-6 relative">
             {/* Vertical Progress Line */}
             <div className="absolute left-[19px] top-4 bottom-4 w-0.5 bg-neutral-100 dark:bg-neutral-800 -z-10" />
               ...
               return (
                 <div key={step.id} className="flex items-start gap-5 group">
                    <div className={cn(
                      ...
                      isActive ? "border-primary shadow-lg scale-110" : isCompleted ? "border-foreground bg-foreground text-background" : "border-border/60 text-muted-foreground/40"
                    )}>
        ...
        <div className="relative z-10 pt-10 border-t border-border/40">
           <div className="flex items-center gap-4">
              <div className="space-y-0.5">
                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Node Identity</p>
                <p className="text-xs font-mono font-bold text-foreground">v1.0.4-ledger</p>
              </div>
              <div className="h-8 w-px bg-border/40" />
              <div className="space-y-0.5">
                <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Encryption</p>
                <p className="text-xs font-bold text-foreground uppercase">AES-256</p>
              </div>
           </div>
        </div>
      </div>
        ...
        <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-xl shrink-0">
          <div className="px-8 lg:px-12 flex h-16 items-center justify-between">
            <div className="flex items-center gap-6">
               ...
               <span className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground">Broadcast Mode</span>
            </div>
            <div className="flex items-center gap-4">
               <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Fee</span>
               <Badge className="bg-green-500/10 text-green-600 border-none px-3 font-mono font-bold text-xs">0.00 ICP</Badge>
            </div>
          </div>
