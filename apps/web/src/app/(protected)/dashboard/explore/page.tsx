...
function DashboardExploreContent() {
	...
	useEffect(() => {
		...
		fetchAllProposals().then((result) => {
			if (cancelled) return;
			setProposals(
				result.success && result.proposals ? result.proposals : [],
			);
		});
	...
	const activeNodes = safeProposals.reduce(
		(acc, p) => acc + (p?.voter_count || 0),
		0,
	);
	const totalPledged = safeProposals.reduce(
