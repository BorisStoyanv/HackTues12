...
export const proposalSchema = z.object({
  ...
  budget_breakdown: z.string().min(20, "Please provide a budget breakdown."),
  executor_name: z.string().min(2, "Executor name is required."),
  execution_plan: z.string().min(50, "Execution plan must be detailed (min 50 chars)."),
  timeline: z.string().min(10, "Timeline is required."),
  expected_impact: z.string().min(50, "Expected impact must be detailed (min 50 chars)."),
  location: z.object({
